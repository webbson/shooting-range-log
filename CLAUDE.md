...
## User guide (keep it current)
`docs/user-guide-sv.pdf` + `-en.pdf` are generated by `npm run guide` from
`docs/guide/content.py` (text + callout coordinates) and `docs/guide/img/<lang>/*.jpg`
(real screenshots). They ship as Tauri resources (`bundle.resources`) and open from the
drawer via the Rust command `open_user_guide` (the opener plugin's JS API is
ACL-scoped and failed silently on Windows; the Rust call is not, and its errors
surface as a notification). **Any change to the checkout, check-in, members or weapons screens must update the
guide in the same commit:** re-capture the affected screenshots with `npm run dev:mock`
(browser UI on :1421 backed by `src/mockIpc.ts`, a fake Tauri IPC with fixture data),
adjust `content.py`, run `npm run guide`, and commit the new PDFs. Screenshots are named
after the screen (`checkin-list.jpg`, `member-edit.jpg`, …); callouts are fractions of the
image, so a re-capture at the same layout needs no coordinate edits.
...
- `src/`: `App.tsx` (providers + routes; `/checkout` remounts on nav click via `location.key`
  ...
  badge, theme + fullscreen toggles (fullscreen = the Rust `set_kiosk` command:
  undecorated, always-on-top, monitor-sized — never OS fullscreen, which flickers
  through a display-mode switch on Windows and kills the WebView's custom cursor),
  fullscreen-only shutdown button with confirm
